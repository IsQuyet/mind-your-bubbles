...
Air Bar Tweaks 让这个值更容易被注意到。你可以决定原版空气条何时显示。
...
Air Bar Tweaks 会在游戏启动后创建这个配置文件：
...
如果安装了 Mod Menu 和 Cloth Config，你也可以在 Mod Menu 的配置界面中修改同一个选项。游戏内配置界面会立即保存更改。
...
Air Bar Tweaks 只控制原版空气条的可见性。它不会修改呼吸机制、空气条贴图、HUD 布局，也不会修改其他 HUD 条，例如：
